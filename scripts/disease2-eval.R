...
inputDir <- paste0(baseDir, "/data/raw/disease2")
outputDir <- paste0(baseDir, "/data/raw/disease2")
figureDir <- paste0(baseDir, "/data/figures/disease2")
...
kp_ps <- ggplot(outputKP_PS, aes(x=k, y=up,
						group=h,
						linetype=factor(h),
						color=factor(h))) +
		geom_line(size=0.9) +
...
kr_ps <- ggplot(outputKR_PS, aes(x=k, y=(1 - rho),
						group=h,
						linetype=factor(h),
						color=factor(h))) +
		geom_line(size=0.9) +
