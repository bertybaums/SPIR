...
data <- NULL
for(h in H){
	info <- fread(paste0(inputDir, "/output-D2-H", h,"-d01.csv"),
			sep=";", header=TRUE)
...
write.table(data, file=paste0(outputDir,"/replication-H-D2.csv"),
...
write.table(summary, file=paste0(outputDir,"/summary-H-D2.csv"),
    append=FALSE, quote=FALSE, sep=";", row.names=FALSE, col.names=TRUE)

## Generate plots
summary <- fread(paste0(inputDir, "/summary-H-D2.csv"), header=TRUE, sep=";")
...
summary <- NULL
for(d in D){
  info <- fread(paste0(inputDir, "/output-D2-H30-d0", d, ".csv"),
                sep=";", header=TRUE)
...
write.table(summary, file=paste0(outputDir,"/summary-d-D2.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE, col.names=TRUE)

## Generate plots
summary <- fread(paste0(inputDir, "/summary-d-D2.csv"), header=TRUE, sep=";")
