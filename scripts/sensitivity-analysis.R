...
inputDir <- paste0(baseDir, "/data/raw/hete")
outputDir <- paste0(baseDir, "/data/figures/hete")
...
colourCount = length(unique(data[which(i == 1 | i == 2 | (i >= 3 & i <= 29)),]$i))
getPalette = colorRampPalette(brewer.pal(9, "Set1"))

ggplot(data[which(i == 1 | i == 2 | (i >= 3 & i <= 29)),],
        aes(x=time, y=(avgI/(avgS+avgP+avgI+avgR))*100,
            group=as.factor(i),
            colour=as.factor(i))) +
    xlab("Time") +
    ...
    scale_color_manual(name = "Payoffs P+I+R",
                       labels = c("Random", "Static",
                               "0.80+0.05+0.90", "0.80+0.05+0.95", "0.80+0.05+1.00",
                               "0.80+0.10+0.90", "0.80+0.10+0.95", "0.80+0.10+1.00",
                               "0.80+0.15+0.90", "0.80+0.15+0.95", "0.80+0.15+1.00",
...
ggplot(data[which(i == 1 | i == 2 | (i >= 30 & i <= 34)),],
  ...
  scale_color_manual(name = "Protection",
                     labels = c("Random", "Static", "0.9", "0.8", "0.7", "0.6", "0.5"),
                     values = c("black", "blue", "green", "red", "orange", "gray", "brown", "magenta", "purple")) +
  theme(axis.title.x = element_text(color = 'black', size = 12, face = 'bold',
                                    margin=margin(t=0.2, unit = "cm")),
        axis.title.y = element_text(color = 'black', size = 16, face = 'bold',
...
ggplot(data[which(i == 1 | i == 2 | (i >= 35 & i <= 39)),],
  ...
  scale_color_manual(name = "Fear",
                     labels = c("Random", "Static", "0.90", "0.95", "1.00", "1.05", "1.10"),
                     values = c("black", "blue", "green", "red", "orange", "gray", "brown", "magenta", "purple")) +
  theme(axis.title.x = element_text(color = 'black', size = 12, face = 'bold',
                                    margin=margin(t=0.2, unit = "cm")),
        axis.title.y = element_text(color = 'black', size = 16, face = 'bold',
...
ggplot(data[which(i == 1 | i == 2 | (i >= 40 & i <= 42)),],
  ...
  scale_color_manual(name = "Decision\nFrequency",
                     labels = c("Random", "Static", "0.01", "0.02", "0.03"),
                     values = c("black", "blue", "green", "red", "orange", "gray", "brown", "magenta", "purple")) +
  theme(axis.title.x = element_text(color = 'black', size = 12, face = 'bold',
                                    margin=margin(t=0.2, unit = "cm")),
        axis.title.y = element_text(color = 'black', size = 16, face = 'bold',
...
ggplot(data[which(i == 1 | i == 2 | (i >= 43 & i <= 48)),],
  ...
  scale_color_manual(name = "Planning\nHorizon",
                     labels = c("Random", "Static", "1", "15", "30", "45", "90", "180"),
